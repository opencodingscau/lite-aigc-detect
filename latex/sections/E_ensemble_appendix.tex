...
The left group contains single models and the right group fixed equal-weight ensembles; no ensemble exceeds LiteSSM-A (dashed teal).\label{fig:ensemble}}
\end{figure}
...
Double-fault and related diversity measures quantify the extent to which classifiers share failures~\cite{kuncheva2003}.
...
After locking the Panel~A protocol, we ran an exploratory matched-generator adaptation probe outside the paper freeze.
M1 denotes the Phase-0 CE-plus-ID-anchor run; M1-v2 extends it with FLUX/SDXL training images and uses \texttt{fixed\_last} selection without UFD hyperparameter search.
Table~\ref{tab:m1v2-app} summarizes descriptive AUCs.

\begin{table}[H]
\caption{Exploratory matched-generator adaptation (not a Panel~A main-table method; selection forbidden on UFD and on held-out SD3.5).
An em dash denotes that the held-out SD3.5 probe was not evaluated for that run.\label{tab:m1v2-app}}
\scriptsize
\setlength{\tabcolsep}{3.5pt}
