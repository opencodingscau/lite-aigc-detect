...
\begin{table}[H]
\caption{Panel~A: controlled lightweight models trained under our frozen from-scratch protocol.
...
LiteSSM-A achieved the highest UFD Macro AUC among the controlled lightweight models while retaining a batch-32 throughput of \textbf{226} images/s (batch-1 p50 \textbf{144.4}\,ms).
LiteSSM-B is slower (p50 \textbf{237.6}\,ms; throughput \textbf{120} images/s) with a slightly lower Macro.
LiteSSM-A is therefore selected as the preferred generalization--efficiency operating point, rather than the model with the lowest absolute latency.
...
\textbf{DALL·E remained close to chance for every detector (0.451--0.519)}, including both SSMs; the failure is not specific to LiteFreqNet.
Figure~\ref{fig:heatmap} visualizes the UFD generator--model matrix.
