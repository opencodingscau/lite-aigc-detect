...
We use DiffusionForensics (DF) images from two content domains present in training: ADM bedroom and Stable Diffusion~v2 CelebA-HQ, each with real and fake folders~\cite{wang2023dire}.
CelebA-HQ was introduced as a high-quality face dataset by Karras et al.~\cite{karras2018progan}; the ADM and latent-diffusion / Stable Diffusion generator families are described by Dhariwal and Nichol~\cite{dhariwal2021adm} and Rombach et al.~\cite{rombach2022ldm}, respectively.
...
The released \texttt{test\_ood.jsonl} preserves the source label and membership of every row in the supplementary pooled set; that set is not used for model selection or as a substitute for UFD Macro.
...
A decision threshold is then fit on validation probabilities by maximizing Youden's $J=\mathrm{TPR}-\mathrm{FPR}$~\cite{youden1950}.
...
\begin{itemize}
\item UnivFD (CLIP ViT-L/14 linear probe)~\cite{ojha2023ufd}; its visual backbone follows CLIP~\cite{radford2021clip};
...
Their reported model-only latency is descriptive only: native preprocessing and training paradigms differ, so Panel~B values are not used for a speed ranking against Panel~A.
...
Code, frozen summaries, and hashes are released at \url{https://github.com/opencodingscau/lite-aigc-detect} (tag \texttt{v1.0.0}) and archived on Zenodo (concept DOI \url{https://doi.org/10.5281/zenodo.21604044}). Data Availability lists original dataset providers, license limits on redistributing raw images, manifest hashes, and checkpoint policy.
