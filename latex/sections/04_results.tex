Unless otherwise noted, point estimates are accompanied by percentile bootstrap 95\% CIs ($B=1000$, seed~42) from the frozen prediction package.
...
Panel~B reaches higher UFD Macro values (UnivFD \textbf{0.948}; NPR \textbf{0.976}) under pretrained or externally released training paradigms.
These results are reported separately because their training settings differ from Panel~A.
...
Point estimates are shown; percentile bootstrap 95\% CIs for ID and UFD Macro are reported in the accompanying text.
UFD Macro is the equal-weight mean across the eight UFD subsets; Worst is the minimum subset AUC.\label{tab:overall}}
\footnotesize
...
Public weights and native preprocessing; not training-matched competitors.
Batch-1 latency is descriptive only and is not used for a speed ranking against Panel~A.\label{tab:external}}
\footnotesize
...
\caption{Accuracy--efficiency operating points: UFD Macro AUC versus batch-1 latency (ms/image, FP32, model-only p50; log $x$-axis).
Left: full Macro range $[0.60,1.00]$ with Panel~B references (NPR, UnivFD) and LiteSSM-A/B labeled; low-latency CNN/FFT points are unmarked.
Right: Panel~A zoom $[0.60,0.80]$ with short tags
A$\star$: LiteSSM-A;
B: LiteSSM-B;
Shuf: ShuffleNetV2-x0.5;
Eff: EfficientNet-B0;
Freq: LiteFreqNet~v2;
Mob: MobileNetV3-Small.
Marker size scales with Params; LiteSSM-A is the preferred Panel~A operating point (highest Macro, not lowest latency).\label{fig:pareto}}
\end{figure}
...
The three Panel~A rows represent the highest-UFD-Macro model (LiteSSM-A), the strongest CNN baseline (EfficientNet-B0), and the frequency-aware model (LiteFreqNet~v2); all retained thresholds are archived with the freeze.
Balanced Acc.\ $=$ $(\mathrm{Sensitivity}+\mathrm{Specificity})/2$.\label{tab:operate}}
\scriptsize
\setlength{\tabcolsep}{3.2pt}
\begin{tabularx}{\textwidth}{@{}P{2.15cm}l c *{4}{R}@{}}
\toprule
\textbf{Model} & \textbf{Split} & \textbf{Thr} & \textbf{Bal.\ Acc.} & \textbf{F1} & \textbf{Sens.} & \textbf{Spec.} \\
\midrule
...
Figure~\ref{fig:confusion} reports confusion counts and within-class rates for LiteSSM-A at the locked Youden threshold ($0.43$).
On ID test the operating point yields TN$=$161, FP$=$39, FN$=$16, TP$=$184 ($n{=}400$); on UFD pooled the same threshold yields TN$=$2748, FP$=$452, FN$=$1619, TP$=$1581 ($n{=}6400$), making the elevated false-negative mass under generator shift explicit.
Appendix~\ref{app:diagnostics} further shows ID threshold sweeps around these operating points and seed-42 training dynamics.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_confusion_litesm_a.png}
\caption{Confusion matrices for LiteSSM-A at the validation Youden threshold ($0.43$) applied unchanged to ID test and UFD (pooled).
Cells report absolute counts (TN/FP/FN/TP) and row-normalized rates; both panels use the same $[0,1]$ color scale.\label{fig:confusion}}
\end{figure}
...
Figure~\ref{fig:qual-bedroom} provides qualitative bedroom examples selected by a predefined disagreement rule (not visual appeal): the top row maximizes $|p_{\mathrm{LiteSSM\text{-}A}}-p_{\mathrm{Eff\text{-}B0}}|$ among cases where LiteSSM-A is correct and EfficientNet-B0 is wrong at their locked thresholds; the bottom row shows high-confidence shared errors.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_domain_gap.png}
\caption{Within-ID AUCs by content domain.
CelebA-HQ is near saturation for all Panel~A models; bedroom AUC is the discriminative axis.
The displayed $y$-axis is restricted to $[0.55,1.02]$ to resolve the bedroom differences.\label{fig:domain}}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig_qual_bedroom.png}
\caption{Qualitative ID bedroom success/failure modes (224$\times$224 crops).
Top: LiteSSM-A correct and EfficientNet-B0 wrong, ranked by maximum score disagreement $|p_{\mathrm{A}}-p_{\mathrm{Eff}}|{>}0$ within bedroom (stratified real/fake).
Bottom: both models wrong, ranked by mean distance to their operating thresholds (highest-confidence shared errors).
Examples were selected by these ranking rules, not by visual appeal; selected paths are archived in \texttt{docs/qualitative\_selection\_meta.json}.
Images are from DiffusionForensics and are shown for academic illustration under the dataset providers' terms.\label{fig:qual-bedroom}}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{figures/fig3_generator_heatmap.png}
...
Appendix~\ref{app:diagnostics} reports score distributions for the DALL·E failure.
...
Figure~\ref{fig:qual-ufd} shows the same contrast with real images: DALL·E fakes nearest to $p{=}0.5$ for LiteSSM-A (NPR still near $1.0$) versus high-confidence Glide$_{100,10}$ detections.
Pairwise Panel~A decision agreement (Cohen's $\kappa$) is reported in Appendix~\ref{app:diagnostics}.
Probability-mean ensembles of the same frozen predictions are reported in Appendix~\ref{app:ensemble}; none exceeds LiteSSM-A on UFD Macro.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig_qual_ufd_dalle_glide.png}
\caption{Qualitative UFD contrast (224$\times$224).
Top: DALL·E fakes ranked by $|p_{\mathrm{LiteSSM\text{-}A}}-0.5|$ (nearest to chance); NPR (ref) scores remain high on the same images.
Bottom: Glide$_{100,10}$ fakes with highest LiteSSM-A scores among detections at thr$=0.43$.
Selection follows these predefined ranking rules (not visual appeal); paths are archived in \texttt{docs/qualitative\_selection\_meta.json}.
Images are from UniversalFakeDetect and are shown for academic illustration under the dataset providers' terms.\label{fig:qual-ufd}}
\end{figure}
...
\caption{Forest plot of ID $\Delta\mathrm{AUC}=\mathrm{AUC}_{Q70}-\mathrm{AUC}_{\mathrm{clean}}$ with paired bootstrap 95\% CIs ($B{=}1000$, seed~42).
...
UFD Macro is reported only for primary main-table checkpoints; ablation rows use ID and OOD Pooled only (UFD Macro not evaluated for these variants).\label{tab:freq-abl}}
\scriptsize
\setlength{\tabcolsep}{4pt}
