...
Table~\ref{tab:seed-app} and Figure~\ref{fig:seed-app} report per-seed scores and sample mean$\pm$std over the three runs (Bessel-corrected, $n{-}1=2$).
LiteSSM-A Macro sample std remains modest ($0.011$); EfficientNet-B0 Macro varies more ($0.027$), so we do \emph{not} replace primary rankings with these means.
...
Squares show mean with sample standard deviation.\label{fig:seed-app}}
\end{figure}

\begin{table}[H]
\caption{ID and UFD Macro AUC across training seeds 42/43/44 (locked recipe).
Mean$\pm$std uses the sample standard deviation over three fixed seeds (descriptive sensitivity set, not an inferential population estimate).\label{tab:seed-app}}
\scriptsize
\setlength{\tabcolsep}{3.5pt}
\begin{tabularx}{\textwidth}{@{}P{2.3cm}l*{4}{R}@{}}
\toprule
\textbf{Model} & \textbf{Metric} & \textbf{s42} & \textbf{s43} & \textbf{s44} & \textbf{Mean$\pm$std} \\
\midrule
LiteSSM-A & ID & 0.946 & 0.939 & 0.939 & $0.941\pm0.004$ \\
LiteSSM-A & UFD Macro & 0.718 & 0.712 & 0.733 & $0.721\pm0.011$ \\
EfficientNet-B0 & ID & 0.929 & 0.923 & 0.930 & $0.927\pm0.003$ \\
EfficientNet-B0 & UFD Macro & 0.667 & 0.674 & 0.624 & $0.655\pm0.027$ \\
\bottomrule
\end{tabularx}
\end{table}
